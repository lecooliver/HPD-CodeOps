...
def remover(args):
    """Removendo containers que estão bind as portas"""
    client = docker.from_env()
    get_all = client.containers.list()
    for cada_container in get_all:
        conectando = client.containers.get(cada_container.id)
        porta = conectando.attrs['PortBindings']['HostPort']
        if isinstance(porta, dict):
            porta.stats_container = porta.stop(cada_container.id)
        print ('Container %s que estava bindando a porta foi removido' %(cada_container.short_id))
